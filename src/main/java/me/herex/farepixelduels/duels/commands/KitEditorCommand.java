...
public class KitEditorCommand implements CommandExecutor {
    ...
    public void openMainGui(Player p) {
        ...
        inv.setItem(12, modeItem(GameMode.COMBO, Material.GOLD_SWORD, "&eCombo Duel"));
        inv.setItem(13, modeItem(GameMode.BOXING, Material.RAW_FISH, "&eBoxing Duel"));
        inv.setItem(14, modeItem(GameMode.OP, Material.DIAMOND_SWORD, "&eOP Duel"));
    ...
    private ItemStack modeItem(GameMode mode, Material material, String name) {
        ...
        lore.add(color("&a" + formatModeName(mode) + " &7."));
        lore.add("");
        lore.add(color("&eClick to edit!"));
        // Hidden mode id line so the click listener can detect what was clicked:
        lore.add(ChatColor.DARK_GRAY + "MODE:" + mode.name());
    ...
    private String formatModeName(GameMode mode) {
        String s = mode.name().toLowerCase(); // classic, bow, boxing...
