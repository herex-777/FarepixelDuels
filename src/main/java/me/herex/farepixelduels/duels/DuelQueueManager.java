...
public class DuelQueueManager {
    ...
    private void tryStart(GameMode mode, DuelType type) {
        ...
        for (Player pl : players) {
            ...
            setLastMode(pl, mode, type); // <-- for play again
            match.addPlayer(pl);
        }
    ...
    private boolean leaveInternal(Player p, boolean teleport) {
        ...
        DuelMatch m = playerMatch.remove(u);
        if (m != null) {
            touched = true;
            playerMode.remove(u);
    ...
    public boolean isInQueue(UUID id) {
        for (java.util.List<UUID> list : queues.values()) {
            if (list.contains(id)) {
                return true;
            }
        }
    ...
    public void startPrivateDuel(Player a, Player b, GameMode mode) {
        if (a == null || b == null || mode == null) return;

        // leave any existing queues/matches first (silent)
        leaveInternal(a, false);
        leaveInternal(b, false);

        DuelType type = DuelType.ONE_VS_ONE;

        java.util.List<Arena> arenas = arenaManager.getReadyArenas(mode, type);
        if (arenas == null || arenas.isEmpty()) {
            a.sendMessage(ColorUtil.cc("&cNo arenas are ready for this mode."));
            b.sendMessage(ColorUtil.cc("&cNo arenas are ready for this mode."));
            return;
        }

        java.util.Random r = new java.util.Random();
        Arena arena = arenas.get(r.nextInt(arenas.size()));

        // ranked = false for /duel (so stats are NOT counted)
        ...
        playerMatch.put(a.getUniqueId(), match);
        playerMatch.put(b.getUniqueId(), match);
        playerMode.put(a.getUniqueId(), mode);
        playerMode.put(b.getUniqueId(), mode);
