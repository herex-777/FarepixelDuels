...
public class DuelMatch {
    ...
    // Boxing hits
    private final Map<UUID, Integer> boxingHits = new HashMap<UUID, Integer>();

    // player who used /leave (don’t show them DEFEAT title)
    ...
    public List<UUID> getAllPlayers() { return new ArrayList<UUID>(players); }
    ...
    private void endMatch(Player winner) {
        ...
        String vTitle = cc(plugin.getConfig().getString("duels.messages.victory_title", "&6&lVICTORY!"));
        String vSub = cc(plugin.getConfig().getString("duels.messages.victory_subtitle", "&7%winner%&f won the Duel!"))
                .replace("%winner%", winnerName);
        // defeat title exists in config, but we do NOT show it anymore

        // Build losers list (for our own end message, not from config)
        List<String> loserNames = new ArrayList<String>();
        for (UUID u : players) {
            if (winnerId != null && u.equals(winnerId)) continue;
            Player pl = Bukkit.getPlayer(u);
            if (pl != null) loserNames.add(pl.getName());
        }
        String losersJoined = loserNames.isEmpty() ? "None" : String.join(", ", loserNames);
        ...
        if (winnerId != null && coinManager != null) {
            ...
            if (amount > 0) {
                try {
                    coinManager.addCoins(winnerId, amount); // adjust name if yours is different
                    Player wp = Bukkit.getPlayer(winnerId);
                    if (wp != null) {
                        wp.sendMessage(ColorUtil.cc("&aYou received &e" + amount + " &acoins for winning!"));
                    }
                } catch (Throwable ignored) {
                    // if method name differs, adjust manually in this block
                }
            }
        }
        ...
        for (UUID u : players) {
            ...
            // Clear any duel items
            p.getInventory().clear();
            p.getInventory().setArmorContents(null);
            p.updateInventory();
            ReturnItemUtil.remove(p, plugin); // just in case
            scoreboardManager.clear(p);

            // Fancy end message (hardcoded)
            p.sendMessage(ColorUtil.cc("&a&m-----------------------------------------------------"));
            p.sendMessage(ColorUtil.cc("                        &fDuels"));
            p.sendMessage(" ");
            p.sendMessage(ColorUtil.cc("               &eWinner: &a" + winnerName));
            p.sendMessage(ColorUtil.cc("               &cLoser(s): &f" + losersJoined));
            p.sendMessage(" ");
            p.sendMessage(ColorUtil.cc("&a&m-----------------------------------------------------"));

            // Titles:
            if (winnerId != null && u.equals(winnerId)) {
                // Winner keeps VICTORY title
                p.sendTitle(vTitle, vSub);
            } else {
                // No DEFEAT title anymore
            }
        }

        // Give "Play Again" item (slot from config, default 4)
        givePlayAgainItem();

        // Give leave/return item after game ends as well
        for (UUID u : players) {
            Player p = Bukkit.getPlayer(u);
            if (p != null) {
                ReturnItemUtil.give(p, plugin);
            ...
            public void run() {
                ...
                for (UUID u : players) {
                    ...
                    if (p != null && lobby != null) {
                        p.teleport(lobby);
                        // you can also show lobby scoreboard here if you want:
                        // scoreboardManager.showLobby(p);
                    }
                }
    ...
    private String getDisplayNameWithPrefix(Player p) {
        ...
        try {
            // Uses PlaceholderAPI via ColorUtil.papi to inject LuckPerms prefix
            base = ColorUtil.papi(plugin, p, "%luckperms_prefix%" + base);
        } catch (Throwable ignored) {
            // If PlaceholderAPI/LuckPerms not present, just show name
        }
        return base;
    }
    ...
    public String getOpponentsOf(Player p) {
        ...
        for (UUID u : players) {
            ...
            if (op != null) {
                String display = getDisplayNameWithPrefix(op); // prefix + name
